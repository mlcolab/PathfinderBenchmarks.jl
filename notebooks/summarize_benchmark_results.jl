...
function compute_performance_metrics(converg_diag, sample_stats)
    ...
    time_total = dropdims(
        sum(sample_stats.time; dims=(:chain, :stage)); dims=(:chain, :stage)
    )
...
function compute_hmc_stats(idata)
    ...
    bfmi_vals = dropdims(mapslices(bfmi, idata.sample_stats.energy; dims=:draw); dims=:draw)
...
function compute_reference_posterior_stats(ref, prob)
    ...
    hessian = stack(
        map(eachslice(ref.x_unc; dims=(:draw, :chain))) do xi
...
function pathfinder_results_to_dataset(pathfinder_results, prob)
    ...
    coords = (param_unc=param_unc_names, param_unc2=param_unc_names, param=param_names)
    ds = cat(
        map(pathfinder_results) do (name, runs)
            data = cat(
                map(runs) do mpf
                    single_path_ds = cat(
                        map(mpf.pathfinder_results) do spf
                            (;
                                draws,
                                fit_distribution,
                                elbo_estimates,
                            ...
                            x_unc = draws
                            x = mapslices(
                                xi -> BridgeStan.param_constrain(prob.model, xi),
                                x_unc;
                                dims=1,
                            )
                    ...
                    merge(
                        single_path_ds,
                        namedtuple_to_dataset(
                            (; x_unc_resampled, x_resampled, pareto_shape);
                            dims,
                            coords,
                            default_dims=(),
                        ),
...
function compute_pathfinder_stats(pathfinder_results)
    ...
    return Dataset((; condition_inv_hessian))
end;
...
function summarize_hmc_benchmark(name)
    ...
    reference_posterior_stats = compute_reference_posterior_stats(reference_posterior, prob)
...
function summarize_benchmark(name)
    return merge(summarize_hmc_benchmark(name), summarize_pathfinder_benchmark(name))
end;
