...
function _covellipse_args(
    (μ, Σ)::Tuple{AbstractVector{<:Real},AbstractMatrix{<:Real}}; n_std::Real
)
    size(μ) == (2,) && size(Σ) == (2, 2) ||
...
begin
    @recipe(CovEllipsePlot, mean, cov) do scene
        return Makie.Attributes(;
            Makie.default_theme(scene, Makie.Poly)..., prob=0.95, n_ellipse_vertices=100
        )
    end
...
ref_draws = let
    draws = DimArray(
        PosteriorDB.load(
            PosteriorDB.reference_posterior(
                PosteriorDB.posterior(pdb, "eight_schools-eight_schools_centered")
            ),
        ),
        Dim{:chain},
    )
    draws_arr = stack(
        map(draws) do chain
            DimArray(
                stack(collect(values(chain))),
                (:draw, Dim{:param}(BridgeStan.param_names(eight_schools.model))),
            )
        end,
    )
    draws_arr_unc = set(
        mapslices(draws_arr; dims=:param) do x
            BridgeStan.param_unconstrain(eight_schools.model, Vector(x))
        end,
        :param => Dim{:param_unc}(BridgeStan.param_unc_names(eight_schools.model)),
    )
end
...
result_banana = let
    ...
    multipathfinder(
        prob_banana, 1_000; rng, ndraws_per_run=1000, nruns=1, init_scale=5, optimizer
    )
end
...
let
    ...
    contourf!(
        ...
        xrange,
        yrange,
        exp ∘ logp_banana ∘ Base.vect;
        levels=0.05:0.05:1,
        mode=:relative,
        colormap=(:plasma, 0.25),
    )
    result = result_banana.pathfinder_results[1]
    niter = length(result.fit_distributions)
    cov1 = nothing
    for (i, dist) in enumerate([
        result.fit_distributions[1],
        result.fit_distributions[2:3:(end - 1)]...,
        result.fit_distributions[end],
    ])
        (; μ, Σ) = dist
        cov1 = covellipseplot!(
            ax,
            ...
            color=(:black, 0.0),
            strokewidth=2,
            strokecolor=(:green, i / niter),
        )
    end
    (; μ, Σ) = result.fit_distribution
    cov2 = covellipseplot!(
        ax, μ, Σ; prob=0.95, color=(:black, 0.0), strokewidth=2, strokecolor=:orange
    )
    tracex = first.(result.optim_trace.points)
    tracey = last.(result.optim_trace.points)
    scat = scatter!(
        ax, result.draws[1, :], result.draws[2, :]; color=(:gray20, 0.2), markersize=5
    )
    scat2 = scatter!(
        ax,
        result_banana.draws[1, :],
        result_banana.draws[2, :];
        color=(:black, 0.8),
        markersize=5,
    )
    ...
    Legend(
        fig[1, 1],
        [lintrace, cov1, cov2, scat, scat2],
        ["L-BFGS path", "fit along path", "final fit", "draws", "resampled draws"];
        labelsize=14,
...
let
    ...
    axis = Axis(
        fig[1, 1]; title="eight_schools_centered", xlabel=L"\theta_1", ylabel=L"\log\tau"
    )
    cont = contourf!(
        axis,
        kde(hcat(vec(θ1ref), vec(τref)));
        levels=0.1:0.05:1,
        mode=:relative,
        colormap=(:plasma, 0.25),
    )
    ...
    Legend(
        fig[1, 1],
        [cont, scat],
        ["reference draws", "Pathfinder draws"];
        tellwidth=false,
        tellheight=false,
        valign=:bottom,
        halign=:right,
